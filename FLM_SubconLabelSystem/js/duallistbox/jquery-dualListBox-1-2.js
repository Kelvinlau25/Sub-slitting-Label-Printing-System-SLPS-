...
*           *callBack                   - call this function after a move/copy event
*
...
(function ($) {
    ...
    $.configureBoxes = function (options) {
        ...
        var index = settings.push({
            ...
            callBack: function (list,hidden,savebutton) {
                var optionsList = '';
                $('#' + list + ' option').each(function (i) {
                    if (optionsList.length > 0) { optionsList += ';'; }
                    optionsList += $(this).text() + ':' + $(this).val();
                });
                $('#' + hidden).val(optionsList);
        ...
        group2.push({
            ...
            hidden: settings[index].selectedValues,
            saveButton: settings[index].saveButton,
            index: index
        });
...
//        $('#' + group.view).empty().append($toSortOptions);
    ...
    function CallBack(group) {
        if (settings[group.index].callBack) { settings[group.index].callBack(settings[group.index].box2View, settings[group.index].selectedValues, settings[group.index].saveButton); }
        return true;
    }
})(jQuery);